...
# Calling the function to clear the terminal
clear()
...
# Reads (to check if new day) then writes over the 
# date.txt file to store date when program is ran.
...
# Constants for graphics/visual separators
LINEBREAK_GRAPHIC = "____________________________________________________"
...
# Variables interacting with database so that they can be 
# used in functions throughout the program.
...
def fed_dog_count_tally():
	...
	# ..probably should've made a function
...
# Main menu to take input and lead user to page based on input.
def main_menu():
    ...
    while True:
        ...
        try:
            ...
            print("7. Exit")
            print(LINEBREAK_GRAPHIC)
            #Takes user to choice, checks if input is valid
            selection = input("\n\nEnter menu choice: \n")
            if selection == "1":
                view_dogs()
                print("\nDogs in shelter\n")
...
def new_dog_menu():
    ...
    def add_new_dog():
        ...
        while True:
            ...
            global dogs, dog_id
            new_dog = {}
			#Checking previous ID in dog_id_count and 
			#writing a new one for current entry
            ...
            #Covering for user errors/better user experience
            if (medical_requirements.lower().startswith("y") and 
                    len(medical_requirements) < 4):
                medical_requirements = "Yes"
            elif (medical_requirements.lower().startswith("n") and 
                    len(medical_requirements) < 3):
                medical_requirements = "No"
                
            elif medical_requirements.lower() == "exit":
                main_menu()
            #Users input becomes what is filled out instead of yes/no. 
			# Possible issue as user may misread and put 
			# details of MDR which is the next prompt.
            else:
            ...
            has_been_fed = input("Has " 
							+ name 
							+ " been fed today?: \n")
							
            if (has_been_fed.lower().startswith("y") and 
					len(has_been_fed) < 5):
                has_been_fed = "Yes"
            elif (has_been_fed.lower().startswith("n") and 
					len(has_been_fed) < 5):
                has_been_fed = "No"

            elif has_been_fed.lower() == "exit":
                main_menu()			
            ...
            #new_dog_count = len(new_dog) <- old code not needed?
...
def edit_dog_info():
    ...
    def edit_dog_info_menu():
        ...
        if selection == "1":
            ...
            if (edit_mdr.lower().startswith("y") and 
					len(edit_mdr) < 5):
                edit_mdr = "Yes"
            elif (edit_mdr.lower().startswith("n") and 
					len(edit_mdr) < 5):
                edit_mdr = "No"
            ...
            if (edit_fed.lower().startswith("y") and 
					len(edit_fed) < 5):
                edit_fed = "Yes"
            elif (edit_fed.lower().startswith("n") and 
					len(edit_fed) < 5):
                edit_fed = "No"
    ...
    if dog_rec_to_edit in db:
        ...
        try:
            ...
            continue_pass = input(
                "Hit 'enter' if correct. Type 'back' to return to choose a"
				" different dog or 'exit' to return to the main menu. \n")
            print(STARBREAK_GRAPHIC)
...
#Update dogs fed status, similar algorithm to edit dog function
def mark_dog_as_fed():
    clear()
    global dogs
    view_dogs()
	...
	#matching user input with doggyID in db
    dog_rec_to_edit = db.get(User.DoggyID == int(dog_to_edit))
    ...
    if dog_rec_to_edit in db:
        ...
        try:
            ...
            if (is_fed.lower().startswith("y") and 
					len(is_fed) < 5):
                is_fed = "Yes"
            elif (is_fed.lower().startswith("n") and 
					len(is_fed) < 5):
                is_fed = "No"
            else:
            ...
            print("\n" + STARBREAK_GRAPHIC)
            print(
                f"Invalid input error '{error_return_to_main}'." 
				"Hit 'Enter' to return to main menu.")
            print("\n" + STARBREAK_GRAPHIC)
            return_to_main()
    ...
    def dogs_to_be_fed_view():
        ...
        try: 
            ...
            print(tabulate.tabulate(to_be_fed_output, headers="keys",
                  tablefmt="grid", maxcolwidths=[None, None],))
		#Exception thats triggered when no results found from db.search
        except IndexError:
            print("\n" + LINEBREAK_GRAPHIC)
            print("\nNo dogs to be fed")
            print(LINEBREAK_GRAPHIC + "\n\n")
...
def remove_dog():
	...
	#Searches to find match of users input to entry in db
    ...
    if dog_rec_to_dlt in db:
        ...
        try:
            ...
            continue_pass = input(
                "Hit 'enter' if correct. Type 'back' to return to "
				"choose a different dog or 'exit' to return to the main menu. \n")
            ...
            if continue_pass == "":
                ...
                elif (continue_pass_confirm.lower().startswith("n") and 
						len(continue_pass_confirm) < 5):
                    remove_dog()
                else:
                    remove_dog()
        ...
        except (ValueError, KeyError, NameError, TypeError, IndexError) as err:
            print(STARBREAK_GRAPHIC)
            value_error_input_rtm = input(
                f"Invalid error {err}. Hit 'enter' to return to menu.\n")
            print(STARBREAK_GRAPHIC)
            main_menu()
    else:
		#Catch anything thats missed
